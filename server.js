...
async function saveImage({ path: target, dataUrl, newName }) {
  ...
  const tmp = `${dest}.botcom-tmp-${process.pid}-${Date.now()}`;
  try {
    const fh = await fsp.open(tmp, 'w');
...
async function serveHtmlPreview(req, res, filePath) {
  ...
  try {
    ...
    const styleBlock = `<style data-botcom-preview>
  html, body { overflow: auto; }
  img, video { max-width: 100%; height: auto; }
</style>`;
    const measureScript = '<script data-botcom-measure>(function(){var l=0;function r(){var w=Math.max(document.documentElement.scrollWidth,document.body?document.body.scrollWidth:0);if(w&&w!==l){l=w;try{parent.postMessage({botcomPreviewWidth:w},"*")}catch(e){}}}addEventListener("load",function(){r();setTimeout(r,300)});addEventListener("resize",r)})()</script>';
    ...
    if (!html.includes('data-botcom-preview')) {
      injectHead(styleBlock);
    }
    if (!html.includes('data-botcom-measure')) {
...
async function readJsonFileSafe(p, maxBytes = 1024 * 1024) {
  try {
    const st = await fsp.stat(p);
    if (!st.isFile()) return { ok: false, error: 'not a file' };
    if (st.size > maxBytes) return { ok: false, error: 'too large', size: st.size };
    return { ok: true, data: JSON.parse(await fsp.readFile(p, 'utf8')), size: st.size, mtime: st.mtimeMs };
  } catch (err) {
    return { ok: false, error: err.message };
  }
}
async function readTextFileSafe(p, maxBytes = 256 * 1024) {
  try {
    const st = await fsp.stat(p);
    if (!st.isFile()) return { ok: false, text: '', error: 'not a file' };
    if (st.size > maxBytes) return { ok: false, text: '', error: 'too large', size: st.size };
    return { ok: true, text: await fsp.readFile(p, 'utf8'), size: st.size, mtime: st.mtimeMs };
  } catch (err) {
...
async function readPidSafe(name) {
  const p = path.join(BOTCOM_ROOTS.mediaOps, 'runtime', 'pids', `${name}.json`);
  const d = await readJsonFileSafe(p, 64 * 1024);
  const pid = d.ok && d.data ? Number(d.data.pid || 0) : 0;
  return { ...fileStatSafe(p), pid: pid || null, running: processAlive(pid), data: d.ok ? d.data : null };
}
function compactLaunchReadiness(data) {
  ...
  for (const section of data.sections || []) {
    for (const item of section.items || []) {
      ...
      if (item.status === 'block') blockers.push(entry);
      else if (item.status === 'warn') warnings.push(entry);
    }
  }
  return {
...
function mobileUrlFromText(text) {
  const m = String(text || '').match(/https?:\/\/[^\s<>"')]+/);
  return m ? m[0] : '';
}
async function botcomStatus() {
  const mediaOps = BOTCOM_ROOTS.mediaOps;
  const workbench = BOTCOM_ROOTS.workbench;
  ...
  const manifestItems = manifest.ok && manifest.data && Array.isArray(manifest.data.items) ? manifest.data.items : [];
  const launchSummary = launch.ok ? compactLaunchReadiness(launch.data) : null;
  const mobilePrimary = mobileUrlFromText(mobileText.text);
  ...
  return {
    ...
    app: {
      ...
      productName: pkg.ok && pkg.data && pkg.data.build ? pkg.data.build.productName : APP_NAME,
      source: 'mit-licensed desktop cockpit',
    },
    workbench: {
      path: workbench,
      dashboard: path.join(workbench, 'dashboard.html'),
      manifest: manifestPath,
      exists: fileStatSafe(workbench).exists,
      generated_at: manifest.ok && manifest.data ? manifest.data.generated_at : '',
      items: manifestItems.length,
      active: manifestItems.filter((x) => x.status === 'active').length,
    ...
    mediaOps: {
      ...
      readme: path.join(mediaOps, 'README.md'),
      ...
      queue: {
        ...
        byStatus: countBy(queueRows, 'status'),
        byPlatform: countBy(queueRows, 'platform'),
        sample: queueRows.slice(0, 6).map((r) => ({
          day: r.day,
      ...
      mobile: {
        primaryUrl: mobilePrimary,
        markdown: mobileMdPath,
        html: path.join(mediaOps, 'runtime', 'mobile_entry.html'),
        qr: path.join(mediaOps, 'runtime', 'mobile_entry_qr.png'),
        qrExists: fileStatSafe(path.join(mediaOps, 'runtime', 'mobile_entry_qr.png')).exists,
      },
    },
    commands: [
      { id: 'launch-readiness', label: '重新生成上线审计', cwd: mediaOps, cmd: 'python3 scripts/media_ops.py launch-readiness --json' },
      { id: 'service-status', label: '查看后台服务状态', cwd: mediaOps, cmd: 'python3 scripts/media_ops.py service --json status all' },
...
function botcomReviewPort() {
  const envPort = Number(process.env.MEDIA_OPS_REVIEW_PORT);
  if (envPort > 0) return envPort;
  const pidFile = path.join(BOTCOM_ROOTS.mediaOps, 'runtime', 'pids', 'review.json');
  try {
    const data = JSON.parse(fs.readFileSync(pidFile, 'utf8'));
    const cmd = Array.isArray(data.cmd) ? data.cmd : [];
    const i = cmd.indexOf('--port');
    const p = Number(i >= 0 ? cmd[i + 1] : 0);
    if (p > 0) return p;
  } catch { /* default below */ }
  return 8787;
}
async function botcomReviewToken() {
  const p = path.join(BOTCOM_ROOTS.mediaOps, 'runtime', 'mobile_entry.md');
  try {
    const text = await fsp.readFile(p, 'utf8');
    const m = text.match(/token=([^\s<>"')]+)/);
    if (m) return m[1];
  } catch { /* env fallback */ }
  return process.env.MEDIA_OPS_REVIEW_TOKEN || '';
}
function localReviewRequest(method, apiPath, body, token) {
  return new Promise((resolve) => {
    const payload = body ? Buffer.from(JSON.stringify(body), 'utf8') : null;
    const req = http.request({
      host: '127.0.0.1',
      port: botcomReviewPort(),
      path: apiPath,
      method,
      headers: {
      ...
      resp.on('end', () => {
        ...
        resolve({ ok: resp.statusCode >= 200 && resp.statusCode < 300, status: resp.statusCode, data });
      });
    });
    req.on('timeout', () => { try { req.destroy(new Error('review server timeout')); } catch { /* */ } });
    req.on('error', (err) => resolve({ ok: false, error: err.message, data: null }));
    if (payload) req.write(payload);
    req.end();
  });
}
async function botcomReviewState() {
  const token = await botcomReviewToken();
  if (!token) return { ok: false, error: 'review token missing; run media_ops.py mobile-entry' };
  return localReviewRequest('GET', '/api/state', null, token);
}
async function botcomReviewAction(body) {
  const action = String(body.action || '');
  const day = String(body.day || '');
  const platform = String(body.platform || 'all');
  const publisher = String(body.publisher || 'smart');
  const allowedActions = new Set(['approve', 'reject', 'publish', 'retry']);
  const allowedDays = new Set(['day01', 'day02', 'day03', 'day04', 'day05', 'day06', 'day07']);
  const allowedPlatforms = new Set(['x', 'xiaohongshu', 'douyin', 'all']);
  const allowedPublishers = new Set(['smart', 'mock', 'x-api', 'x-api-short', 'x-api-thread', 'x-api-video-text', 'douyin-browser', 'xiaohongshu-browser']);
  if (!allowedActions.has(action)) return { ok: false, status: 400, data: { error: 'invalid action' } };
  if (!allowedDays.has(day) || !allowedPlatforms.has(platform)) return { ok: false, status: 400, data: { error: 'invalid day/platform' } };
  if (!allowedPublishers.has(publisher)) return { ok: false, status: 400, data: { error: 'invalid publisher' } };
  const token = await botcomReviewToken();
  if (!token) return { ok: false, status: 400, data: { error: 'review token missing; run media_ops.py mobile-entry' } };
  return localReviewRequest('POST', `/api/${action}`, {
    day,
    platform,
    publisher,
    notes: String(body.notes || 'BotCom Workbench').slice(0, 1000),
    auto_publish_after_approve: !!body.auto_publish_after_approve,
  }, token);
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/botcom/status') {
      return sendJSON(res, 200, await botcomStatus());
    }
    if (p === '/api/botcom/review-state') {
      return sendJSON(res, 200, await botcomReviewState());
    }
...
server.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.error(`\n  ⚠️  端口 ${PORT} 已被占用——${APP_NAME} 或其他本机服务很可能已经在运行了。`);
    console.error(`      直接打开浏览器访问  http://localhost:${PORT}  就行；`);
    console.error(`      想另开一个，换端口：BOTCOM_WORKBENCH_PORT=4571 node server.js\n`);
  } else {
    console.error('\n  启动失败：', err.message, '\n');
  }
  process.exit(1);
