 ...
 */
(() => {
  const saved = localStorage.getItem('botcom_lang');
  const sys = (navigator.language || 'en').toLowerCase();
  const lang = saved === 'zh' || saved === 'en' ? saved : (sys.startsWith('zh') ? 'zh' : 'en');
  window.botcomLang = lang;

  // 语言切换：记到 localStorage（渲染层）+ config.json（Electron 菜单读），刷新生效
  window.botcomSetLang = (l) => {
    localStorage.setItem('botcom_lang', l);
  ...
  const wireToggle = () => {
    ...
    el.onclick = () => window.botcomSetLang(lang === 'zh' ? 'en' : 'zh');
  };
  ...
  const HAN = /[㐀-鿿「」（）：；！？…·]/;
  const dict = () => window.BOTCOM_DICT || {};
  const rules = () => window.BOTCOM_DICT_RULES || [];
  const trOne = (core) => {
    const hit = dict()[core];
