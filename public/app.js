...
const state = {
  ...
  theme: localStorage.getItem('botcom_theme') || 'botcom',
  entries: [], project: null, history: [],
  view: localStorage.getItem('botcom_view') || 'grid',
  gridSize: localStorage.getItem('botcom_gridsize') || 'md',
  sort: localStorage.getItem('botcom_sort') || 'name',
  showHidden: localStorage.getItem('botcom_hidden') === '1',
  filter: '', selected: null, cursor: -1, cols: 1, visible: [],
  favorites: [], recentOpened: [], recentMode: false, skillsMode: false,
  previewW: Number(localStorage.getItem('botcom_preview_w')) || 480,
  previewH: Number(localStorage.getItem('botcom_preview_h')) || 340,
  sidebarCollapsed: localStorage.getItem('botcom_sidebar_collapsed') === '1',
  sidebarW: Math.min(420, Math.max(190, Number(localStorage.getItem('botcom_sidebar_w')) || 248)),
  muted: localStorage.getItem('botcom_muted') === '1', // WOW4 提示音静音开关
...
function updateWatches() {
  if (!window.botcomFs) return;
  const dirs = new Set();
  if (state.cwd) dirs.add(state.cwd);
  if (typeof term !== 'undefined') term.sessions.forEach((s) => { if (s.startDir) dirs.add(s.startDir); });
  if (window.botcomFs.watchSet) window.botcomFs.watchSet([...dirs]);
  else window.botcomFs.watch(state.cwd); // 旧版主进程兜底
}
...
function bindItem(el, e) {
  ...
  el.addEventListener('dragstart', (ev) => {
    ...
    ev.dataTransfer.setData('application/x-botcom-path', e.path);
...
async function dropFilesInto(fileList, dir) {
  if (!window.botcomDrop || !dir) { toast('该环境不支持拖入保存', true); return; }
  const files = [...(fileList || [])];
  if (!files.length) return;
  let saved = 0, lastPath = null;
  for (const f of files) {
    const src = window.botcomDrop.pathForFile(f);
    let r;
    if (src) r = await window.botcomDrop.copyInto(src, dir).catch(() => null);
    else r = await window.botcomDrop.saveInto(dir, f.name, await f.arrayBuffer()).catch(() => null);
...
function makeDraggablePath(el, p) {
  ...
  el.addEventListener('dragstart', (ev) => {
    ...
    ev.dataTransfer.setData('application/x-botcom-path', p);
    ev.dataTransfer.effectAllowed = 'copy';
  });
}
...
function renderHtmlPreview(data, meta) {
  ...
  const onMsg = (ev) => {
    ...
    const w = ev.data && ev.data.botcomPreviewWidth;
...
function renderPreviewActions(e) {
  ...
  box.innerHTML = '';
  const clip = window.botcomClipboard;
...
async function copyImage(p) { const r = await window.botcomClipboard.copyImage(p); toast(r.ok ? '已复制图片，可粘贴到其它应用' : '复制图片失败：' + (r.error || ''), !r.ok); }
async function copyFile(p) { const r = await window.botcomClipboard.copyFile(p); toast(r.ok ? '已复制文件，可在访达里粘贴' : '复制文件失败', !r.ok); }
...
function bindSidebarResizer() {
  ...
  window.addEventListener('mouseup', () => {
    ...
    apply();
    localStorage.setItem('botcom_sidebar_w', state.sidebarW);
  });
}
...
function ensureFileAreaSize() {
  ...
  if (mb.classList.contains('fm-squeezed')) {
    ...
    localStorage.setItem('botcom_term_squeeze', '0');
    const termSize = Math.max(minTerm, available - desiredFile - RESIZER);
    if (isBottom) {
      panel.style.height = termSize + 'px';
      localStorage.setItem('botcom_term_h', termSize);
    } else {
      panel.style.width = termSize + 'px';
      localStorage.setItem('botcom_term_w', termSize);
    }
    animateLayout();
    term.fitActive();
    return;
...
function toggleSidebar(force) {
  ...
  localStorage.setItem('botcom_sidebar_collapsed', state.sidebarCollapsed ? '1' : '0');
...
const shotTray = {
  el: null, timer: null,
  init() {
    if (!window.botcomShot) return; // 浏览器版没有截屏监听
    window.botcomShot.onNew((m) => this.show(m));
  },
  show(m) {
    this.dismiss();
    const el = document.createElement('div');
...
// BotCom Workbench只备料——把整理偏好、过往整理历史、工作约定写成 brief 文件，agent 读完先摊方案，
...
const botcomDashboard = {
  ...
  async show() {
    ...
    ov.innerHTML = `<div class="input-dialog botcom-dialog">
      <div class="botcom-head">
        <div>
          <div class="botcom-kicker">BotCom OS Desktop</div>
          <div class="input-title">一人公司经营控制台</div>
        </div>
        <div class="botcom-head-actions">
          <button class="ghost-btn" data-act="refresh">${ic('clock', 'currentColor', 13)} 刷新</button>
          <button class="ghost-btn" data-act="close">关闭</button>
        </div>
      </div>
      <div class="botcom-body"><div class="cmdk-loading">读取一人公司 OS 状态…</div></div>
    </div>`;
    document.body.appendChild(ov);
    const close = () => { if (this.ov) this.ov.remove(); this.ov = null; document.removeEventListener('keydown', onKey, true); };
    const onKey = (ev) => { if (ev.key === 'Escape') { ev.preventDefault(); close(); } };
    ov.onclick = (ev) => { if (ev.target === ov) close(); };
    ov.querySelector('[data-act=close]').onclick = close;
    ov.querySelector('[data-act=refresh]').onclick = () => this.refresh();
    document.addEventListener('keydown', onKey, true);
    await this.refresh();
  },
  async refresh() {
    ...
    if (!body) return;
    body.innerHTML = '<div class="cmdk-loading">刷新经营状态…</div>';
    try {
      const [status, review] = await Promise.all([
        api('/api/botcom/status'),
        api('/api/botcom/review-state').catch((err) => ({ ok: false, error: err.message })),
      ]);
      this.data = status;
      this.review = review && review.ok ? review.data : null;
      this.reviewError = review && !review.ok ? (review.error || review.data?.error || 'review server unavailable') : '';
      this.render();
    } catch {
      body.innerHTML = '<div class="empty-state">BotCom 状态读取失败</div>';
    }
  },
  tone(status) {
    if (status === 'pass' || status === 'ready' || status === 'running') return 'good';
    if (status === 'warn' || status === 'review') return 'warn';
    if (status === 'block' || status === 'blocked' || status === 'stopped') return 'bad';
    return '';
  },
  pill(label, status) {
    return `<span class="botcom-pill ${this.tone(status)}">${escapeHtml(label)}</span>`;
  },
  serviceLine(name, svc) {
    const running = svc && svc.running;
    const pid = svc && svc.pid ? ` · PID ${svc.pid}` : '';
    return `<div class="botcom-service ${running ? 'on' : 'off'}"><span>${escapeHtml(name)}</span><b>${running ? '运行中' : '未运行'}${pid}</b></div>`;
  },
  shortPath(p) {
    if (!p) return '';
    return state.home ? String(p).replace(state.home, '~') : String(p);
  },
  platformLabel(p) {
    return { x: 'X', xiaohongshu: '小红书', douyin: '抖音', all: '全平台' }[p] || p;
  },
  platformDir(p) {
    return { x: 'x', xiaohongshu: 'xhs', douyin: 'douyin' }[p] || p;
  },
  mediaPath(...parts) {
    const base = this.data?.mediaOps?.path || '';
    return [base.replace(/\/$/, ''), ...parts.filter(Boolean)].join('/');
  },
  queueRows() {
    const rows = this.review && Array.isArray(this.review.rows) ? this.review.rows : [];
    const order = { day01: 1, day02: 2, day03: 3, day04: 4, day05: 5, day06: 6, day07: 7, x: 1, xiaohongshu: 2, douyin: 3 };
    return rows.slice().sort((a, b) => (order[a.day] || 99) - (order[b.day] || 99) || (order[a.platform] || 99) - (order[b.platform] || 99));
  },
  contentText(row) {
    const day = this.review && this.review.days && this.review.days[row.day];
    if (!day) return row.content_id || '';
    return row.platform === 'x'
      ? day.x?.short_post || day.x?.thread || ''
      : row.platform === 'xiaohongshu'
        ? day.xiaohongshu?.note || day.xiaohongshu?.cards_copy || ''
        : day.douyin?.caption || day.douyin?.script || '';
  },
  contentPreview(row) {
    const text = this.contentText(row);
    return String(text).replace(/\s+/g, ' ').trim().slice(0, 160);
  },
  queueSection(queue) {
    if (this.reviewError) {
      return `<section class="botcom-card"><div class="botcom-card-title">内容 / 获客审核队列</div><div class="botcom-issue bad"><b>Review server 不可用</b><span>${escapeHtml(this.reviewError)}</span></div></section>`;
    }
    const rows = this.queueRows();
    if (!rows.length) {
      return `<section class="botcom-card"><div class="botcom-card-title">内容 / 获客审核队列</div><div class="botcom-sub">未读取到 review server 队列；摘要队列=${queue.rows || 0}</div></section>`;
    }
    const current = rows.filter((r) => r.day === 'day01').length ? 'day01' : rows[0].day;
    const groups = [...new Set(rows.map((r) => r.day))];
    return `<section class="botcom-card">
      <div class="botcom-card-title">内容 / 获客审核队列 · 可直接操作</div>
      <div class="botcom-sub">默认按钮只改本地队列状态；“批准并自动 / 发布/准备”会调用内容运营 worker，真实平台仍受 publish policy 和平台登录/凭据限制。</div>
      <div class="botcom-day-tabs">${groups.map((d) => `<button data-botcom-day="${escapeHtml(d)}" class="${d === current ? 'active' : ''}">${escapeHtml(d.toUpperCase())}</button>`).join('')}</div>
      <div class="botcom-queue">
        ${rows.map((r) => `<div class="botcom-qrow ${r.day === current ? '' : 'hidden'}" data-q-day="${escapeHtml(r.day)}">
          <div class="botcom-qmain">
            <div class="botcom-qtitle"><b>${escapeHtml(r.day.toUpperCase())}</b><span>${escapeHtml(this.platformLabel(r.platform))}</span>${this.pill(r.status || 'unknown', r.status)}</div>
            <div class="botcom-qid">${escapeHtml(r.content_id || '')}</div>
            <div class="botcom-qcopy">${escapeHtml(this.contentPreview(r))}</div>
            <div class="botcom-qtools">
              <button data-q-open="pack" data-day="${escapeHtml(r.day)}" data-platform="${escapeHtml(r.platform)}">打开文案包</button>
              <button data-q-open="assets" data-day="${escapeHtml(r.day)}" data-platform="${escapeHtml(r.platform)}">打开素材目录</button>
              <button data-q-copy data-day="${escapeHtml(r.day)}" data-platform="${escapeHtml(r.platform)}">复制完整文案</button>
            </div>
            ${r.last_error ? `<div class="botcom-qerr">${escapeHtml(r.last_error)}</div>` : ''}
          </div>
          <div class="botcom-qactions">
            <button data-review-action="approve" data-day="${escapeHtml(r.day)}" data-platform="${escapeHtml(r.platform)}">批准</button>
            <button data-review-action="approve-auto" data-day="${escapeHtml(r.day)}" data-platform="${escapeHtml(r.platform)}">批准并自动</button>
            <button data-review-action="publish" data-day="${escapeHtml(r.day)}" data-platform="${escapeHtml(r.platform)}">发布/准备</button>
            <button data-review-action="reject" data-day="${escapeHtml(r.day)}" data-platform="${escapeHtml(r.platform)}">驳回</button>
          </div>
        </div>`).join('')}
      </div>
    </section>`;
  },
  openQueuePath(kind, day, platform) {
    const p = kind === 'assets'
      ? this.mediaPath('publish_queue', day, 'assets')
      : this.mediaPath('publish_queue', day, this.platformDir(platform));
    if (!p) return toast('media-ops 路径不存在', true);
    if (this.ov) this.ov.remove();
    this.ov = null;
    navigate(p);
  },
  copyQueueText(day, platform) {
    const row = this.queueRows().find((r) => r.day === day && r.platform === platform);
    if (!row) return toast('队列项不存在', true);
    this.copy(String(this.contentText(row)).trim(), '已复制完整文案');
  },
  async reviewAction(action, day, platform) {
    const map = {
      approve: ['approve', false, '批准'],
      'approve-auto': ['approve', true, '批准并自动发布/准备'],
      publish: ['publish', false, '发布/准备'],
      reject: ['reject', false, '驳回'],
      retry: ['retry', false, '重试'],
    };
    const spec = map[action];
    if (!spec) return;
    const [apiAction, auto, label] = spec;
    const ok = await confirmDialog(`确认${label} ${day}/${this.platformLabel(platform)}？`);
    if (!ok) return;
    const result = await apiPost('/api/botcom/review-action', {
      action: apiAction,
      day,
      platform,
      publisher: 'smart',
      auto_publish_after_approve: auto,
      notes: 'BotCom Workbench desktop review',
    });
    if (!result.ok) {
      toast(result.data?.error || result.error || `${label}失败`, true);
    } else {
      toast(`${label}完成`);
    }
    await this.refresh();
  ...
  copy(text, msg) {
    if (!text) return toast('没有可复制内容', true);
    navigator.clipboard?.writeText(text).then(() => toast(msg || '已复制')).catch(() => {
      window.prompt('复制下面内容', text);
    });
  },
  openUrl(url) {
    if (!url) return toast('入口 URL 不存在', true);
    window.open(url, '_blank');
  },
  run(cmd) {
    if (!cmd) return;
    if (!term.available()) return toast('需要桌面版内嵌终端执行命令', true);
    term.runInDir(cmd.cwd, cmd.cmd, cmd.label || 'BotCom 命令已在终端启动');
    if (this.ov) this.ov.remove();
    this.ov = null;
  },
  moduleTone(tone) {
  ...
  companyModules(m, w, lr, growth, queue, services) {
    ...
    const rows = Number(queue.rows || 0);
    const channels = Object.keys(queue.byPlatform || {}).length;
    const blockers = (lr.blockers || []).length;
    const warnings = (lr.warnings || []).length;
    const growthSignals = (growth.recommendations || []).length + (growth.next_tests || []).length;
  ...
  render() {
    ...
    const recs = growth.recommendations || [];
    const tests = growth.next_tests || [];
    const cmds = d.commands || [];
    const platformCounts = queue.byPlatform || {};
    const statusCounts = queue.byStatus || {};
    const modules = this.companyModules(m, w, lr, growth, queue, services);
    body.innerHTML = `
      <div class="botcom-grid">
        <section class="botcom-card botcom-hero">
          <div class="botcom-card-title">经营状态</div>
          <div class="botcom-main-status ${this.tone(lr.overall)}">${escapeHtml(lr.overall || 'unknown')}</div>
          <div class="botcom-pills">
            ${this.pill(`通过 ${lr.counts?.pass || 0}`, 'pass')}
            ${this.pill(`警告 ${lr.counts?.warn || 0}`, warnings.length ? 'warn' : 'pass')}
        ...
        <section class="botcom-card">
          ...
          <div class="botcom-num">${queue.rows || 0}</div>
          <div class="botcom-sub">状态：${Object.entries(statusCounts).map(([k, v]) => `${escapeHtml(k)}=${v}`).join(' · ') || '—'}</div>
          <div class="botcom-sub">平台：${Object.entries(platformCounts).map(([k, v]) => `${escapeHtml(k)}=${v}`).join(' · ') || '—'}</div>
        </section>
        <section class="botcom-card">
          <div class="botcom-card-title">自动化服务</div>
          ${this.serviceLine('手机审批台', services.review)}
          ${this.serviceLine('发布准备 Worker', services.worker)}
        </section>
        <section class="botcom-card">
      ...
      <div class="botcom-two">
        <section class="botcom-card">
          <div class="botcom-card-title">当前阻塞</div>
          ${blockers.length ? blockers.map((b) => `<div class="botcom-issue bad">
            <b>${escapeHtml(b.label || b.id || '')}</b>
            <span>${escapeHtml(b.detail || '')}</span>
            ${b.next_step ? `<em>${escapeHtml(b.next_step)}</em>` : ''}
          </div>`).join('') : '<div class="botcom-ok">无阻塞，进入可发布状态。</div>'}
          ${warnings.length ? `<div class="botcom-card-title sub-title">警告</div>${warnings.map((b) => `<div class="botcom-issue warn"><b>${escapeHtml(b.label || b.id || '')}</b><span>${escapeHtml(b.detail || '')}</span></div>`).join('')}` : ''}
        </section>
        <section class="botcom-card">
          <div class="botcom-card-title">下一步动作</div>
          ${(next.length ? next : ['先完成手机审批台和内容队列配置；确认发布策略、平台登录态、收入/客户数据入口后，再扩大自动化范围。']).map((x) => `<div class="botcom-next">${escapeHtml(x)}</div>`).join('')}
        </section>
      </div>

      <section class="botcom-card">
        <div class="botcom-card-title">经营复盘建议</div>
        <div class="botcom-list">${recs.slice(0, 4).map((x) => `<div>${escapeHtml(x)}</div>`).join('') || '<div>等待发布后的真实指标。</div>'}</div>
        ${tests.length ? `<div class="botcom-tests">${tests.slice(0, 3).map((x) => `<span><b>${escapeHtml(x.experiment || '')}</b>${escapeHtml(x.test || '')}</span>`).join('')}</div>` : ''}
      </section>

      <section class="botcom-card">
        <div class="botcom-card-title">执行命令</div>
        <div class="botcom-cmds">
          ${cmds.map((c) => `<button data-cmd="${escapeHtml(c.id)}">${escapeHtml(c.label)}</button>`).join('')}
        </div>
        <div class="botcom-sub mono">手机入口：${escapeHtml(m.mobile?.primaryUrl || '未生成')}</div>
        <div class="botcom-sub mono">内容运营中台：${escapeHtml(this.shortPath(m.path))}</div>
      </section>`;
...
function bindResizer() {
  ...
  window.addEventListener('mouseup', () => {
    ...
    localStorage.setItem('botcom_preview_w', state.previewW);
    localStorage.setItem('botcom_preview_h', state.previewH || 340);
  });
}
...
function bindTerminalResizer() {
  ...
  window.addEventListener('mouseup', () => {
    ...
    apply(); fitNow();
    const panel = $('#terminal-panel');
    localStorage.setItem('botcom_term_squeeze', squeeze ? '1' : '0');
    if (term.dock === 'bottom') localStorage.setItem('botcom_term_h', parseInt(panel.style.height, 10) || 280);
    else localStorage.setItem('botcom_term_w', parseInt(panel.style.width, 10) || 480);
  });
}
...
function bindEvents() {
  ...
  syncMute();
  muteBtn.onclick = () => { state.muted = !state.muted; localStorage.setItem('botcom_muted', state.muted ? '1' : '0'); syncMute(); if (!state.muted) playChime('tick'); };
  ...
  tp.addEventListener('dragover', (ev) => {
    ...
    if (!t.includes('Files') && !t.includes('application/x-botcom-path') && !t.includes('text/plain')) return;
  ...
  tp.addEventListener('drop', async (ev) => {
    ...
    if (files.length && window.botcomDrop) {
      for (const f of files) {
        let p = window.botcomDrop.pathForFile(f);
        if (!p) {
          const r = await window.botcomDrop.saveTemp(f.name, await f.arrayBuffer()).catch(() => null);
          if (r && r.ok) p = r.path;
        }
    ...
    const sk = ev.dataTransfer.getData('application/x-botcom-skill');
    if (sk) { invokeSkillInTerm(sk); return; }
    const p = ev.dataTransfer.getData('application/x-botcom-path') || ev.dataTransfer.getData('text/plain');
    if (p) term.insertPath(p);
  });
  ...
  // 只接「外部文件」拖入（dataTransfer 里有 Files）；工作台内部拖拽不带 Files，不受影响。
  ...
  $('#toggle-hidden').onchange = (e) => { state.showHidden = e.target.checked; localStorage.setItem('botcom_hidden', state.showHidden ? '1' : '0'); renderFiles(); };

  $('#sort-seg').querySelectorAll('button').forEach((b) => {
    b.classList.toggle('active', b.dataset.sort === state.sort);
    b.onclick = () => { state.sort = b.dataset.sort; localStorage.setItem('botcom_sort', state.sort); $('#sort-seg').querySelectorAll('button').forEach((x) => x.classList.toggle('active', x === b)); renderFiles(); };
  });
  $('#view-seg').querySelectorAll('button').forEach((b) => {
    b.classList.toggle('active', b.dataset.view === state.view);
    b.onclick = () => { state.view = b.dataset.view; localStorage.setItem('botcom_view', state.view); $('#view-seg').querySelectorAll('button').forEach((x) => x.classList.toggle('active', x === b)); updateGridSizeVisibility(); renderFiles(); };
  });
  $('#gridsize-seg').querySelectorAll('button').forEach((b) => {
    b.classList.toggle('active', b.dataset.size === state.gridSize);
    b.onclick = () => { state.gridSize = b.dataset.size; localStorage.setItem('botcom_gridsize', state.gridSize); $('#gridsize-seg').querySelectorAll('button').forEach((x) => x.classList.toggle('active', x === b)); renderFiles(); };
  });
...
function applyTheme(skin, rerender = true) {
  ...
  localStorage.setItem('botcom_theme', skin);
...
const term = {
  ...
  dock: localStorage.getItem('botcom_term_dock') || 'bottom',
  available() { return !!(window.botcomPty && window.Terminal && !window.__noXterm); },
  // 每套皮肤一整套手调 ANSI 主题——暗皮肤暗终端、亮皮肤亮终端，不再出现「暖纸里嵌黑块」
  themes: {
    botcom: {
      background: '#070b1a', foreground: '#dce7ff', cursor: '#7cf7d4', cursorAccent: '#07111f', selectionBackground: '#7c5cff40',
      black: '#111a2e', red: '#ff6b8a', green: '#7cf7d4', yellow: '#ffd166', blue: '#6ea8ff', magenta: '#b58cff', cyan: '#55d6ff', white: '#dce7ff',
      brightBlack: '#667399', brightRed: '#ff85a0', brightGreen: '#9fffe2', brightYellow: '#ffe08a', brightBlue: '#8dbbff', brightMagenta: '#c9a8ff', brightCyan: '#7ce4ff', brightWhite: '#ffffff',
    },
    terminal: {
  ...
  open() {
    ...
    localStorage.setItem('botcom_term_open', '1');
    if (!localStorage.getItem('botcom_term_draghint')) { localStorage.setItem('botcom_term_draghint', '1'); setTimeout(() => toast('提示：把左侧文件 / 文件夹拖进终端，即插入路径喂给 agent'), 700); }
  },
  close() {
    ...
    localStorage.setItem('botcom_term_open', '0');
  },
  applyDock() {
    const mb = $('#main-body');
    ...
    mb.classList.toggle('fm-squeezed', termOpen && localStorage.getItem('botcom_term_squeeze') === '1');
    const panel = $('#terminal-panel');
    if (this.dock === 'bottom') { panel.style.height = (Number(localStorage.getItem('botcom_term_h')) || 280) + 'px'; panel.style.width = ''; }
    else { panel.style.width = (Number(localStorage.getItem('botcom_term_w')) || 480) + 'px'; panel.style.height = ''; }
    applyPreviewSize(); // 预览随 dock 翻转轴向
    this.fitActive();
  },
  setDock(d) {
    if (this.maximized) this.toggleMax(false); // 铺满下切布局看不出任何变化，先退出铺满让分屏可见
    animateLayout(); this.dock = d; localStorage.setItem('botcom_term_dock', d); this.applyDock();
  },
  ...
  openInDir(dir) {
    ...
    this.applyDock();
    $('#btn-terminal').classList.add('active');
    localStorage.setItem('botcom_term_open', '1'); // 右键/一键开终端也记住开合，和 open/close 对称
  ...
  async isPlainShell(s) {
    try {
      const r = await window.botcomPty.proc(s.id);
  ...
  input(id, d) {
    ...
    window.botcomPty.input(id, d);
  },
  // 点终端里的文件名/路径 → 结合 cwd + 回扫 scrollback + 搜索定位真实文件，在BotCom Workbench里打开
  ...
  async openTermPath(id, raw, tail, rowHint) {
    ...
    if (isRel) {
      try { const r = await window.botcomPty.cwd(id); if (r && r.ok && r.cwd) cwd = r.cwd; } catch { /* */ }
  ...
  async locateCwd() {
    if (!this.active) return;
    const r = await window.botcomPty.cwd(this.active);
  ...
  async refreshCwd(s, force) {
    ...
    s._cwdAt = now;
    try {
      const r = await window.botcomPty.cwd(s.id);
  ...
  async newTab(cwdOverride) {
    ...
    const r = await window.botcomPty.spawn({ id, cwd: startDir, cols: xterm.cols, rows: xterm.rows });
    ...
    xterm.onResize(({ cols, rows }) => { sess.lastInput = Date.now(); window.botcomPty.resize(id, cols, rows); }); // resize 引发的 TUI 重绘不算 agent 干活
    window.botcomPty.resize(id, xterm.cols, xterm.rows); // spawn 等待期间 fit 过的 resize 事件无人监听会丢：补发一次对齐 PTY
    // 识别终端输出里的文件路径 → hover 高亮 + 点击在BotCom Workbench打开
  ...
  async respawn(sess) {
    ...
    const r = await window.botcomPty.spawn({ id: sess.id, cwd: sess.startDir || state.cwd, cols: sess.xterm.cols, rows: sess.xterm.rows });
  ...
  closeTab(id) {
    ...
    try { window.botcomPty.kill(id); } catch { /* */ }
  ...
  notify(s, title, body) {
    try {
      ...
      const fire = () => {
        ...
        n.onclick = () => {
          try { if (window.botcomWin) window.botcomWin.focus(); else window.focus(); } catch { /* */ }
...
const usagePanel = {
  ...
  open() { return localStorage.getItem('botcom_usage_open') === '1'; },
  apply() {
    const on = this.open();
  ...
  bind() {
    $('#usage-toggle').onclick = () => {
      localStorage.setItem('botcom_usage_open', this.open() ? '0' : '1');
      this.apply();
    };
    this.apply();
  },
  ...
  bind(area) {
    ...
    area.querySelectorAll('.sk-row').forEach((r) => {
      r.addEventListener('dragstart', (ev) => {
        ...
        if (!it) return;
        ev.dataTransfer.setData('application/x-botcom-skill', it.name);
...
const mona = {
  editor: null, _p: null,
  themeFor: { botcom: 'botcom-dark', terminal: 'botcom-dark', warm: 'botcom-paper', editorial: 'botcom-editorial' },
  themeName() { return this.themeFor[state.theme] || 'botcom-dark'; },
  ...
  defineThemes(m) {
    m.editor.defineTheme('botcom-dark', { base: 'vs-dark', inherit: true, rules: [], colors: { 'editor.background': '#0b0c0a', 'editor.foreground': '#d6dac9', 'editorLineNumber.foreground': '#4a4d42', 'editorCursor.foreground': '#cdf24b', 'editor.selectionBackground': '#cdf24b33', 'editor.lineHighlightBackground': '#ffffff08' } });
    m.editor.defineTheme('botcom-paper', { base: 'vs', inherit: true, rules: [], colors: { 'editor.background': '#ece2d2', 'editor.foreground': '#4a3f30', 'editorLineNumber.foreground': '#b3a589', 'editorCursor.foreground': '#cc785c', 'editor.selectionBackground': '#cc785c33', 'editor.lineHighlightBackground': '#00000008' } });
    m.editor.defineTheme('botcom-editorial', { base: 'vs', inherit: true, rules: [], colors: { 'editor.background': '#eae5d8', 'editor.foreground': '#1a1a1a', 'editorLineNumber.foreground': '#9a958a', 'editorCursor.foreground': '#ff433d', 'editor.selectionBackground': '#ff433d22', 'editor.lineHighlightBackground': '#00000008' } });
  },
...
// pty 数据回流（全局一次）
if (window.botcomPty) {
  window.botcomPty.onData(({ id, data }) => { const s = term.sessions.find((x) => x.id === id); if (s) { s.xterm.write(data); term.markBusy(s); } });
  window.botcomPty.onExit(({ id }) => {
...
if (window.botcomFs) {
  ...
  window.botcomFs.onChanged(({ dir, filename }) => {
...
async function init() {
  ...
  if (window.botcomEnv && window.botcomEnv.isDesktopApp) document.documentElement.classList.add('desktop');
  ...
  if (localStorage.getItem('botcom_term_open') === '1' && term.available()) term.open();
  maybeShowGuide();
  bindUpdateNotice();
}
// 新版本提示：主进程查到 GitHub 有新 Release 时右下角弹胶囊，引导去下载页（不强更不打扰）
function bindUpdateNotice() {
  if (!window.botcomUpdate) return;
  const show = ({ version, url }) => {
    if (localStorage.getItem('botcom_skip_ver') === version || document.querySelector('.update-pill')) return;
    ...
    bar.querySelector('.up-go').onclick = () => { window.botcomUpdate.open(url); bar.remove(); };
    bar.querySelector('.up-x').onclick = () => { localStorage.setItem('botcom_skip_ver', version); bar.remove(); };
  };
  window.botcomUpdate.onAvailable(show);
  // 主进程启动 6 秒就推送，init 加载大目录时这里可能还没注册监听——补拉一次，错过的推送不丢
  if (window.botcomUpdate.get) window.botcomUpdate.get().then((m) => { if (m) show(m); }).catch(() => {});
}
init();
