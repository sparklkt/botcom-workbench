'use strict';
/**
 * BotCom Workbench — Electron main process.
 *
 * Reuses the local Node backend and adds an embedded node-pty terminal so
 * coding agents and media-ops commands can run inside the same desktop app.
 */
...
process.env.BOTCOM_NO_OPEN = '1';
process.env.BOTCOM_APP_NAME = process.env.BOTCOM_APP_NAME || 'BotCom Workbench';
process.env.BOTCOM_WORKBENCH_PORT = process.env.BOTCOM_WORKBENCH_PORT || '4570';
const PORT = Number(process.env.BOTCOM_WORKBENCH_PORT) || 4570;
require('../server.js');

// node-pty 是原生模块，需 electron-rebuild 编译过；未就绪时终端能力降级但 app 仍可用
let pty = null;
try { pty = require('node-pty'); }
catch (e) { console.error('[botcom] node-pty 未就绪（跑 npm run rebuild）：', e.message); }
...
app.whenReady().then(() => {
  // 开发模式下 macOS 默认显示 Electron 图标；打包后由 electron-builder 的 icon 接管。
  if (process.platform === 'darwin' && app.dock) {
    try { app.dock.setIcon(nativeImage.createFromPath(path.join(__dirname, '..', 'build', 'icon.png'))); } catch { /* */ }
  }
  app.setName('BotCom Workbench');
  buildMenu();
  createWindow();
  startShotWatch();
  // 默认不自动检查 release；如需启用，设置 BOTCOM_ALLOW_UPSTREAM_UPDATE=1 和 BOTCOM_RELEASE_REPO=owner/repo。
  if (process.env.BOTCOM_ALLOW_UPSTREAM_UPDATE === '1') {
...
const RELEASE_REPO = process.env.BOTCOM_RELEASE_REPO || '';
const REL_PAGE = RELEASE_REPO ? `https://github.com/${RELEASE_REPO}/releases/latest` : '';
async function fetchLatestRelease() {
  ...
  try {
    const res = await net.fetch(`https://api.github.com/repos/${RELEASE_REPO}/releases/latest`, {
      headers: { 'User-Agent': 'botcom-workbench', Accept: 'application/vnd.github+json' },
    });
    if (res.ok) {
  ...
  const res = await net.fetch(REL_PAGE, { headers: { 'User-Agent': 'botcom-workbench' } });
...
// 界面语言：用户手动选过的存在 ~/.botcom-workbench/config.json，没选过跟随系统。
function uiLang() {
  try {
    const c = JSON.parse(fs.readFileSync(path.join(os.homedir(), '.botcom-workbench', 'config.json'), 'utf8'));
...
function buildMenu() {
  ...
  const template = [
    ...(isMac ? [{ label: 'BotCom Workbench', submenu: [
      { role: 'about', label: M('关于 BotCom Workbench', 'About BotCom Workbench') },
      { label: M('检查更新…', 'Check for Updates…'), click: () => checkUpdate({ manual: true }) },
      { type: 'separator' },
      { role: 'hide', label: M('隐藏 BotCom Workbench', 'Hide BotCom Workbench') }, { role: 'hideOthers', label: M('隐藏其他', 'Hide Others') }, { role: 'unhide', label: M('全部显示', 'Show All') },
      { type: 'separator' },
      { role: 'quit', label: M('退出 BotCom Workbench', 'Quit BotCom Workbench') },
    ] }] : []),
...
ipcMain.handle('pty:spawn', (e, { id, cwd, cols, rows }) => {
  ...
  const env = { ...process.env, TERM: 'xterm-256color', BOTCOM_WORKBENCH: '1' };
...
ipcMain.handle('drop:save', (e, { name, buf }) => {
  try {
    const dir = path.join(app.getPath('temp'), 'botcom-drops');
